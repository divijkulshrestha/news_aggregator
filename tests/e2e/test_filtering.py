...
class TestFiltering:
    ...
    def test_should_filter_articles_by_category(self, page: Page):
        ...
        tech_button = page.locator('.filter-btn[data-category="technology"]')
        tech_button.click()
        
        # Check that Technology button is now active
        expect(tech_button).to_have_class("filter-btn active")
        
        # Check that All button is no longer active
        expect(page.locator('.filter-btn[data-category="all"]')).not_to_have_class("filter-btn active")
    ...
    def test_should_switch_between_different_categories(self, page: Page):
        ...
        categories = ["technology", "world", "cricket", "all"]
        
        for category in categories:
            button = page.locator(f'.filter-btn[data-category="{category}"]')
            button.click()
            expect(button).to_have_class("filter-btn active")
            time.sleep(0.5)
    ...
    def test_should_combine_category_and_time_filters(self, page: Page):
        ...
        page.click('.filter-btn[data-category="technology"]')
        time.sleep(0.5)
        ...
        expect(page.locator('.filter-btn[data-category="technology"]')).to_have_class("filter-btn active")
